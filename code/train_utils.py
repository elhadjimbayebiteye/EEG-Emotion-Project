...
print(f"[train_utils] Device utilisé : {device}")
...
def train_onecycle_amp(
    ...
    plot_name_prefix="training_plot",
    batch_size=128,
    epochs=100,
    max_lr=1.5e-3,
    ...
    print(f"\n[train_onecycle_amp] Entraînement — {num_channels} canaux\n")
    ...
    plt.close()
    print("Courbes sauvegardées :", png_path)

    return model, hist, best_eval_acc

# =========================
# 5) Entraînement modèle de l’article CompactEmotionCNNArticle
# =======================

import torch
import torch.nn as nn
from torch.utils.data import DataLoader
from models_conv_emotion import CompactEmotionCNNArticle   # IMPORTANT

def train_emotion_cnn_article(
    X_tr, y_tr,
    X_val, y_val,
    num_channels,
    seq_len,
    ...
    train_loader = DataLoader(
        EEGDataset(X_tr, y_tr, augment=False),
        batch_size=batch_size,
        shuffle=True,
        num_workers=2,
        pin_memory=True,
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} | Train Loss {tr_loss:.4f} Acc {tr_acc:.2f}% | Val Loss {v_loss:.4f} Acc {v_acc:.2f}%")
